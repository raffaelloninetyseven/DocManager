...
class DocManager_FileHandler {
    ...
    public function uploadFile($file_data, $document_id) {
        if (!isset($file_data['tmp_name']) || !is_uploaded_file($file_data['tmp_name'])) {
            error_log('DocManager: File non valido o non caricato - ' . print_r($file_data, true));
            return false;
        }
        
        if (!$this->validateFile($file_data)) {
            error_log('DocManager: Validazione file fallita per ' . $file_data['name']);
            return false;
        }
        ...
        if (!move_uploaded_file($file_data['tmp_name'], $file_path)) {
            error_log('DocManager: Impossibile spostare file in ' . $file_path . ' - Errore: ' . error_get_last()['message']);
            return false;
        }
        ...
        if (file_exists($file_path)) {
            ...
            error_log('DocManager: File non esiste dopo move_uploaded_file: ' . $file_path);
            return false;
        }
        ...
        error_log('DocManager: File caricato con successo - ' . $unique_filename . ' (ID: ' . $file_id . ')');
        return $result;
    }
    ...
    private function validateFile($file_data) {
        ...
        if ($file_data['size'] > $max_size_bytes) {
            error_log('DocManager: File troppo grande: ' . $file_data['size'] . ' bytes (max: ' . $max_size_bytes . ')');
            return false;
        }
    ...
    public function getFileInfo($document_id) {
        ...
        if (!$file_type && function_exists('finfo_open')) {
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
